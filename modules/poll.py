...
class PollButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if previous_vote is not None and previous_vote == self.option_index:
            await interaction.response.send_message(
                "You already voted for that choice.",
                ephemeral=True
            )
            return
        ...
        embed = self.cog.build_poll_embed(poll)
        await interaction.response.edit_message(
            embed=embed,
            view=self.cog.build_poll_view(poll)
        )
...
class Poll(commands.Cog):
    ...
    def build_poll_embed(self, poll: dict) -> discord.Embed:
        ...
        embed = discord.Embed(
            ...
            description=(
                "Choose one of the buttons below to vote.\n"
        ...
        if ended:
            ...
            if not winners:
                ...
                winner_text = (
                    f"🏆 **Winner:** Choice {winners[0] + 1} — "
                    f"**{poll['options'][winners[0]]}** with `{max_votes}` vote(s)"
                )
            else:
                tied_choices = ", ".join(
                    f"Choice {i + 1} — **{poll['options'][i]}**" for i in winners
                )
    ...
    async def poll_watcher(self):
        polls = list(self.get_polls().values())
        now = time.time()
    ...
    @app_commands.command(
        name="poll",
        description="Create a poll with up to 5 choices and a clear time setting"
    )
    ...
    @app_commands.command(
        name="pollresults",
        description="See the current or final results of a poll"
    )
    @app_commands.describe(
        poll_id="The poll ID you want to check"
    )
    ...
    @app_commands.command(
        name="endpoll",
        description="End a poll before its timer runs out"
    )
    @app_commands.describe(
        poll_id="The poll ID to end early"
    )
    async def endpoll(self, interaction: discord.Interaction, poll_id: str):
        ...
        if not is_creator and not is_admin:
            await interaction.response.send_message(
                "❌ Only the poll creator or a server admin can end this poll.",
                ephemeral=True
            )
            return
